...
// US2 happy path — Escenario 2 de quickstart.md
// Reproduce: jubilado con DNI en padrón con cotiza_papel=true ingresa SOLO con
// DNI (sin legajo), recibe magic link, autentica, queda con tipo='jubilado',
// legajo=null y session válida.
...
const EMAIL = `happy-sl-${DNI}+${RUN_ID}@test.local`
...
describe('Integración: flujo sin_legajo happy path (US2)', () => {
  beforeAll(cleanupAll)
  afterAll(cleanupAll)
  beforeEach(cleanupAll)

  it('crea afiliado tipo=jubilado con legajo=null, despacha email, audita y permite autenticar', async () => {
    // 1. Solicitar magic link sin legajo
    ...
    // 2. Verificar afiliado creado con tipo='jubilado' y legajo=null
    const admin = makeServiceRoleClient()
    ...
    // 5. Simular click del magic link y verificar sesión + RLS self-select
    ...
    // 6. El propio afiliado puede leer su fila (RLS self-select)
    const { data: ownAfiliado, error } = await userClient
      .from('afiliados')
      .select('dni, tipo, legajo')
      .single()
    expect(error).toBeNull()
    expect(ownAfiliado).toMatchObject({
      dni: DNI,
      tipo: 'jubilado',
      legajo: null,
    })
