...
export function PendienteActions({ solicitudId, dni }: Props) {
  const [showRechazar, setShowRechazar] = useState(false)
  const [aproState, aproAction] = useFormState(aprobarSolicitud, initial)
  const [rechState, rechAction] = useFormState(rechazarSolicitud, initial)
    ...
    <div className="flex flex-col gap-2">
      ...
      {!showRechazar ? (
        ...
        <form action={rechAction} className="flex flex-col gap-2">
          ...
          >
            Motivo del rechazo (lo verá el solicitante DNI {dni}):
          </label>
          <textarea
